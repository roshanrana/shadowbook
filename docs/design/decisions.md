...
## D-029 — A broker kill must not kill the ledger, on the commit path either

**Status:** accepted (implementation, 2026-09-04) — **defect found by the first real-cluster sweep; extends D-025**
**Context:** D-025 classified fetch errors so the ledger would survive a broker going away. The **commit** path was left untouched, and the first sweep against real Redpanda died in six of nine runs, always ~2m30s in — the second scheduled kill. franz-go reports a commit issued to a dying broker as *"broker closed the connection immediately after a request was issued, which happens when SASL is required but not provided: is SASL missing?"*, its generic text for a handshake-time drop, which reads convincingly like a misconfiguration. One run showed the truth plainly: `dial tcp [::1]:29092: actively refused` — that is redpanda-2, killed on schedule.
**Decision:** `KafkaConsumer.Commit` wraps retriable and connection errors as `broker.ErrTransient`, and the consumer treats a transient commit failure as "skip this batch, it will be redelivered" rather than as fatal.
**Consequences:** Skipping is also the *correct* semantics, not merely the survivable one. For mode A the contract is that the offset is durable before the effect is attempted; applying after a failed commit would give at-least-once behaviour under an at-most-once label. For B, C and D the effects are already applied, and an uncommitted offset means redelivery — which is exactly the condition each mode is defined by. **The lesson is the generalisable part:** fixing the fetch path and not the commit path left the same defect live on a second code path for a week, and only a real cluster could reach it. When an error class is reclassified, every call site that can raise it must be revisited at the same time.

## D-030 — A run that did not drain is not a measurement

**Status:** accepted (implementation, 2026-09-04)
**Context:** `waitForDrain` capped total wait at three minutes and returned an error if the consumer was still applying when the cap expired. At 240,000 movements per run that happened constantly — the consumer works through a backlog at a few hundred a second — and everything it had not yet reached was recorded as loss. One run reported **122,886 "lost" records that were sitting on the broker, intact**. Every fixed parameter agreed across those runs, so the existing guards would have rendered the table without complaint.
**Decision:** The cap now bounds **stalls**, not progress: a consumer still applying is given time (to a 20-minute ceiling that exists only so a wedged run cannot hang a sweep); one that has stopped applying with records outstanding is done, and what it did not take is genuinely lost. Every artefact records `Drained`, and `Table` refuses any set containing a run that did not drain.
**Consequences:** The failure this prevents is the dangerous kind — a plausible table of numbers, internally consistent, describing nothing. It also surfaced a real performance fact worth stating in its own right: on a Docker Desktop/WSL2 host the consumer applies roughly **280 movements/s**, so NFR-1a's 1,000/s chaos rate is not attainable for the consumer path on that hardware. The ablation compares configurations *to each other* under identical chaos, so a lower rate is a legitimate experiment; what is not legitimate is running above what the consumer can sustain and reporting the resulting backlog as loss.

## D-031 — Isolation is per experiment, not per run: the broker outlives the process

**Status:** accepted (implementation, 2026-09-04) — **defect found by the second real-cluster sweep; extends D-022**
**Context:** D-022 gave every run its own database, consumer group and topic, having learned that the per-run database was not enough. Run ids repeat *between* sweeps — `payday-C-1` every time — so the topic and group names did too. The second sweep therefore attached to the first sweep's topics and consumer groups, and consumed 240,000 leftover records from an experiment that had already been discarded. Mode C reported **44,746 distinct movements applied against 36,000 sent**, which is arithmetically impossible; A and B concealed the same contamination because their counts are capped at `sent`, so the surplus surfaced in their duplication column instead and looked like a plausible finding.
**Decision:** Every sweep generates an id, mixed into every topic and consumer group name, recorded in each artefact, and included in the fixed-parameter key so two sweeps can never be folded into one table. `measure` additionally refuses any run where distinct movements applied exceeds movements sent, naming reused topics and groups as the cause.
**Consequences:** The general lesson, now seen three times in this project: **whatever outlives the process is what contaminates the next run.** The database was isolated before the log; the log was isolated per run before it was isolated per sweep. Each time, the surviving state produced results that were internally consistent and wrong. The impossible-arithmetic check is the cheap generic defence — a run that applies more than it sent is not a finding under any interpretation, and catching it costs one run rather than a whole sweep.

## D-032 — Configuration D is implemented but cannot be verified without a real cluster

**Status:** accepted (implementation, 2026-09-04) — **completes T-051's implementation half**
**Context:** D is the transactional configuration. `broker.KafkaTransactionalConsumer` uses franz-go's `GroupTransactSession`, so offsets are committed inside a Kafka transaction rather than as a plain commit, and `cmd/ledger` selects it whenever the mode is D. Verifying it locally is impossible: `kfake` implements the transaction *request types* but not the entry point — `handleInitProducerID` returns `UNKNOWN_SERVER_ERROR` for any request carrying a transactional id, above a literal `// TODO: Transactional IDs`.
**Decision:** Ship the implementation, and record the limitation in a test that **skips with the reason** rather than one that passes against a path the broker never took. The ship report lists D as implemented-but-unverified until it runs against Redpanda.
**Consequences:** What D buys is narrower than its name suggests, and the report says so: the transaction makes the offset commit atomic with anything produced in the same session, but it cannot enclose a PostgreSQL write. D is therefore C plus a transactional offset commit, and its exactly-once property still rests on the inbox primary key exactly as C's does. The difference it does make is at the rebalance boundary — a plain commit landing during a rebalance can be accepted by a coordinator that is no longer authoritative, where the transactional session aborts and the batch is redelivered instead. That is a real but narrow distinction, and it is the honest reason D is worth measuring rather than assuming.

## D-033 — Latency is measured where the architecture permits it, and left blank where it does not

**Status:** accepted (implementation, 2026-09-04)
**Context:** The Finding 2 latency columns rendered as `not measured`. The obvious fix — infer per-record timings from commit timestamps — would put a number in the column that nobody observed.
**Decision:** Measure it exactly, from data the system already keeps. The `inbox` records `applied_at` per `message_id`, a message id encodes its sequence number, and the driver records the wall-clock produce time per sequence. Joining them gives true end-to-end latency: produced → acknowledged by the cluster → consumed → committed to PostgreSQL. Negative samples (container clock skew) are dropped rather than clamped, since clamping biases the percentiles downward. Percentiles are nearest-rank, so every reported figure is a value that was actually observed rather than an interpolation between two.
**Consequences:** Only C and D can be measured this way — A and B keep no inbox and record nothing tying an effect back to a movement. That is the same architectural fact as the net-versus-exact counts, surfacing a second time, and it is worth stating as a result in its own right: **without an inbox the system is not measurable from its own data.** Not a limitation of the harness; a property of the configuration.
